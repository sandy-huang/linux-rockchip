...
static bool kbase_replay_fault_check(struct kbase_jd_atom *katom)
{
	 ...
	 */
	if (katom->event_code == BASE_JD_EVENT_TERMINATED) {
		return false;
	} else if (katom->event_code == BASE_JD_EVENT_JOB_WRITE_FAULT) {
		return true;
	} else if (katom->event_code == BASE_JD_EVENT_FORCE_REPLAY) {
		katom->event_code = BASE_JD_EVENT_DATA_INVALID_FAULT;
		return true;
	} else if (katom->event_code != BASE_JD_EVENT_DATA_INVALID_FAULT) {
	...
	while (job_header) {
		 ...
		 * DATA_INVALID_FAULT */
		if ((katom->event_code == BASE_JD_EVENT_DATA_INVALID_FAULT) &&
		   (JOB_SOURCE_ID(job->exception_status) == JOB_POLYGON_LIST)) {
			err = true;
