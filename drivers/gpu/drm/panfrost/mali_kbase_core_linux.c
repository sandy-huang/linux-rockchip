...
static int kbasep_kds_allocate_resource_list_data(struct kbase_context *kctx, struct base_external_resource *ext_res, int num_elems, struct kbase_kds_resource_list_data *resources_list)
{
	...
	if (resources_list->kds_access_bitmap == NULL) {
...
static long kbase_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (copy_from_user(&msg, (void __user *)arg, size) != 0) {
	...
	if (copy_to_user((void __user *)arg, &msg, size) != 0) {
 ...
 * policy.
 */
static DEVICE_ATTR(power_policy, 0644, show_policy, set_policy);
 ...
 * policy.
 */
static DEVICE_ATTR(core_availability_policy, 0644, show_ca_policy, set_ca_policy);
 ...
 */
static DEVICE_ATTR(core_mask, 0644, show_core_mask, set_core_mask);
...
static DEVICE_ATTR(soft_event_timeout, 0644,
 ...
 */
static DEVICE_ATTR(js_timeouts, 0644, show_js_timeouts, set_js_timeouts);
...
static DEVICE_ATTR(gpuinfo, 0444, kbase_show_gpuinfo, NULL);
...
static DEVICE_ATTR(dvfs_period, 0644, show_dvfs_period,
		set_dvfs_period);
...
static DEVICE_ATTR(pm_poweroff, 0644, show_pm_poweroff,
		set_pm_poweroff);
...
static DEVICE_ATTR(reset_timeout, 0644, show_reset_timeout,
		set_reset_timeout);
...
static DEVICE_ATTR(mem_pool_size, 0644, show_mem_pool_size,
		set_mem_pool_size);
...
static DEVICE_ATTR(mem_pool_max_size, 0644, show_mem_pool_max_size,
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	scnprintf(kbdev->devname, DEVNAME_SIZE, "mali%d", kbase_dev_nr);
